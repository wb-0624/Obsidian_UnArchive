...
如：鼠标悬浮。已访问和未访问。获得焦点时等。

```css
selector:pseudo-class {      //选择器:伪类状态
  property: value;          //声明
}
```
...
| :hover          | 鼠标悬浮                                                                            |
| :active         | 已选择                                                                              |
| :hover p        | 悬浮时显示p元素，类似提示的效果，其他伪类同理                                       |
| p:first-child   | 匹配是其他元素的第一个子元素的p元素                                                 |
| p i:first-child | 匹配所有p元素的第一个i元素  <br> p i 表示后代选择，限定第一个子元素后代             |
| p:first-child i | 先看p i，匹配p元素的所有i子元素，同时p元素有限定条件，p是其他任何元素的第一个子元素 |
| :lang(en)       | 选择lang = "en"的元素                                                               |
| :focus          | 获得焦点，通常为input被选中时                                                       |
| required        | 选择指定了"required"属性的元素                                                      |

### 伪元素选择器

类似于伪类。

伪类用于元素的特殊状态，伪元素用于元素的特殊部分。
...
相对于css要自己定义设计。tailwind可以说已经预设好一系列样式，只需要简单的导入后，就可以直接使用。
